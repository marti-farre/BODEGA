...
class OnlineRLDefenseSelector(OpenAttack.Classifier):
    ...
    def __init__(self, victim, action_space=None, seed: int = 42,
                 ...
                 label_source: str = 'oracle', reward_mode: str = 'hard',
                 mv_k: int = 7, mv_perturb_prob: float = 0.1):
        ...
        # Pseudo-label estimator for true-online mode. Independent wrapper so its
        # output isn't affected by whichever action the policy selects for defence.
        # Uses a different seed from the action-space MV to keep the two RNGs decoupled.
        self.mv_estimator = None
        if self.label_source == 'mv7':
            self.mv_estimator = bodega_get_defense(
    ...
    def observe_result(self, true_label: int, prediction_after_attack: int):
        ...
        if self.label_source == 'mv7':
            ...
            # Oracle: gold label available (upper-bound baseline).
