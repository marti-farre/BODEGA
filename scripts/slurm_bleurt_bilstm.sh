#!/bin/bash
# SLURM array job: BiLSTM attack experiments with BLEURT scorer
# All defenses × 4 tasks × 4 attackers = 448 jobs (array 0-447)
# Index: i = task_idx*112 + attacker_idx*28 + defense_idx
#
# Submit with: cd ~/BODEGA && sbatch scripts/slurm_bleurt_bilstm.sh

#SBATCH -J bleurt_bilstm
#SBATCH -p medium
#SBATCH --gres=gpu:1
#SBATCH --mem=32G
#SBATCH -c 4
#SBATCH --array=0-447
#SBATCH -o logs/bleurt_bilstm_%A_%a.out
#SBATCH -e logs/bleurt_bilstm_%A_%a.err
...
echo "[$i] BiLSTM+BLEURT | $TASK | $ATTACK | $DEFENSE (param=$PARAM)"
