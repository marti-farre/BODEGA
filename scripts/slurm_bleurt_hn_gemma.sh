#!/bin/bash
# SLURM array job: Gemma HN-only with BLEURT + 500 query limit
#
# Submit with: cd ~/BODEGA && sbatch scripts/slurm_bleurt_hn_gemma.sh
...
#SBATCH -c 4
#SBATCH --array=0-111
#SBATCH -o logs/hn_gemma_%A_%a.out
#SBATCH -e logs/hn_gemma_%A_%a.err
...
echo "[$i] GEMMA+BLEURT+LIMITED | $TASK | $ATTACK | $DEFENSE (param=$PARAM) | max_queries=$MAX_QUERIES"
